...
(defn assoc-if-interesting
  ([m k v]
     (assoc-if-interesting
      m k v #(or (and (number? %) (< 0 %))
                 (and (coll? %) (not (empty? %))))))
  ([m k v func]
     (if (func v)
       (assoc m k v)
...
(defrecord FileInfo []
  ...
  (toString [this]
    (str (-> (or (-> this :ns name)
         ...
         "." (-> this :type name)
         (let [m (-> {}
                     (assoc-if-interesting :clj  (-> this :dep-clj count))
                     (assoc-if-interesting :cljs (-> this :dep-cljs count))
                     (assoc-if-interesting :classes (-> this :classes count))
                     (assoc-if-interesting :imports (-> this :dep-imports count)))]
           (if (empty? m) "" m)))))
